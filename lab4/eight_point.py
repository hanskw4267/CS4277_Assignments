...
def decompose_e(E, K, data1, data2):
    ...
    def depth(X, P):
        ...
        depth = (sign * w) / (np.linalg.norm(m3))
        if (depth >= 0):
            return True
        else:
            return False
    ...
    for i in range(4):
        pts_in_front = 0
        for pt in range(15):
            A = [data1[pt, 0] * P[2] - P[0],
                 data1[pt, 1] * P[2] - P[1],
                 data2[pt, 0] * possible_p_prime[i][2] -
                 possible_p_prime[i][0],
                 data2[pt, 1] * possible_p_prime[i][2] - possible_p_prime[i][1]]
            A = np.array(A)

            _, _, vh = np.linalg.svd(A)
            X = vh[-1, :] / vh[-1, -1]

            # check if point X is in front of camera
            depth1 = depth(X, P)
            depth2 = depth(X, possible_p_prime[i])
            if (depth1 and depth2):
                pts_in_front += 1

        # if maximum amt of pts are in front using this pose
        if pts_in_front >= max_pts:
            max_pts = pts_in_front
